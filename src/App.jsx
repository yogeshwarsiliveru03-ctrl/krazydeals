...
function ImageUploadField({ value, onChange, accentColor }) {
  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      const img = new Image();
      img.onload = () => {
        // Smaller max size + adaptive quality keeps each image safely
        ...
        // If still too large, compress further in a couple of passes
...
export default function App() {
  ...
  // Every visitor's browser subscribes to the same "products" collection.
  // When you (the admin) add/edit/delete a product, it updates instantly
  // for everyone currently viewing the site -- no refresh needed.
  useEffect(() => {
    const unsub = onSnapshot(
      collection(db, "products"),
      (snapshot) => {
        if (snapshot.empty) {
          // First time ever: seed the cloud database with the starter products
      ...
      (err) => {
        ...
        // Fallback so the store still shows something even if Firebase isn't configured yet
        setProducts(defaultProducts);
        setLoaded(true);
      }
    );
        ...
        <div className="mx-auto max-w-[1440px]">
          <div className="rounded-3xl px-3 py-2 flex items-center justify-between gap-3"
            ...
            <div className="flex items-center gap-2.5 flex-shrink-0">
              {/* Premium Krazy Deals Logo - designed in Canva */}
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col leading-none">
                  <span className="font-black tracking-tight text-gray-900" style={{ fontSize:"15px", letterSpacing:"-0.02em" }}>KRAZY</span>
                  <span className="font-black tracking-tight" style={{ fontSize:"15px", letterSpacing:"-0.02em", background:"linear-gradient(135deg,#F26522,#FC2779)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>DEALS</span>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-10">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2.5">
                    <a href="https://instagram.com/krazydeals" target="_blank" rel="noopener noreferrer"
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-[11px]">@krazydeals</p>
                      </div>
                    </a>

                    <a href="https://t.me/krazydeals" target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-3 group">
                      <div className="w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover:scale-110"
                    ...
                    <a href="mailto:krazydeals@gmail.com"
                      className="flex items-center gap-3 group">
                      <div className="w-9 h-9 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover:scale-110"
                        style={{ background:"linear-gradient(135deg,#F26522,#FC2779)" }}>
                        <svg viewBox="0 0 24 24" className="w-4 h-4 fill-white">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-[11px]">krazydeals@gmail.com</p>
                      </div>
                    </a>
                  </div>
                </div>
