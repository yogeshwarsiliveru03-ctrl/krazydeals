...
import { PLATFORMS, CAT_THEMES, CATEGORIES, ADMIN_PASSWORD, defaultProducts, detectPlatform, Field, ImageUploadField, Stars, Badge, ShopBtn, CatPill, ProductCard, SkeletonCard, StatsBar, FloatingOrbs, DealTicker, ProductModal, SettingsPanel, ScrollReveal, ProductPage, BottomNav } from "./AppParts";
...
export default function App() {
  ...
  return (
    ...
    <div className="min-h-screen font-sans relative" style={{ background: darkMode ? "linear-gradient(160deg,#0f0f1a 0%,#1a1a2e 50%,#0d0d1f 100%)" : "linear-gradient(160deg,#f8f9ff 0%,#fef3fb 50%,#f0f9ff 100%)" }}>
      ...
      {/* -- NAVBAR -- */}
      <nav className="sticky top-0 z-40 px-3 pt-3 pb-2" style={{ transition:"padding 0.3s ease" }}>
        <div className="mx-auto max-w-[1440px]">
          <div className="rounded-3xl px-3 py-2 flex items-center justify-between gap-3"
            style={{
              background: scrolled ? (darkMode?"rgba(20,20,32,0.95)":"rgba(255,255,255,0.95)") : (darkMode?"rgba(20,20,32,0.4)":"rgba(255,255,255,0.5)"),
              backdropFilter:"blur(20px)",
              boxShadow: scrolled ? "0 4px 30px rgba(0,0,0,0.12), 0 0 0 1px rgba(255,255,255,0.9)" : "0 2px 10px rgba(0,0,0,0.04)",
      ...
      {/* -- HERO -- */}
      <div className="max-w-[1440px] mx-auto px-5 sm:px-8 pt-8 pb-6 relative" style={{ zIndex:1, paddingBottom: view==="store" ? "100px" : "24px" }}>
        {view==="store" ? (
        ...
        {filtered.length===0 ? (
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5"
            style={{
              ...
              opacity: catTransition ? 0 : 1,
              transform: catTransition ? "translateY(8px)" : "translateY(0)",
      ...
      {/* -- BOTTOM NAV (mobile, store view only) -- */}
      {view==="store" && (
        <BottomNav
          activeTab={bottomTab}
          onTabChange={(tab)=>{
            setBottomTab(tab);
            if(tab==="home"){ setActiveCategory("All"); setActiveSubCat("All"); window.scrollTo({top:0,behavior:"smooth"}); }
            if(tab==="search"){ document.querySelector('input[type="text"]')?.focus(); }
            if(tab==="categories"){ window.scrollTo({top:250,behavior:"smooth"}); }
          }}
          darkMode={darkMode}
          cartCount={0}
        />
