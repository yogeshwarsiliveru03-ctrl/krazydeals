  ...
  const Field=({label,k,type="text",placeholder=""})=>(
    <div className="flex flex-col gap-1.5">
      ...
      <input type={type} value={form[k]||""} onChange={e=>set(k,type==="number"?+e.target.value:e.target.value)}
        ...
        onFocus={e=>{e.target.style.borderColor=plat.color;e.target.style.background="rgba(255,255,255,0.9)";}}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div className="w-full sm:max-w-lg rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col max-h-[92vh]"
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 flex flex-col gap-4">
          <Field label="Product name *" k="name" placeholder="e.g. boAt Rockerz 450"/>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Selling price Rs. *" k="price" type="number" placeholder="1299"/>
            <Field label="Original price Rs." k="originalPrice" type="number" placeholder="3999"/>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Rating (0-5)" k="rating" type="number" placeholder="4.2"/>
            <Field label="Reviews count" k="reviews" type="number" placeholder="18432"/>
          </div>
          <Field label="Product image URL" k="image" placeholder="https://..."/>
          <Field label="Short description" k="description" placeholder="Key features in one line"/>
        ...
        <div className="mx-auto max-w-[1440px]">
          <div className="rounded-3xl px-3 py-2 flex items-center justify-between gap-3"
            ...
            <div className="flex items-center gap-2">
              <button onClick={()=>setView(v=>v==="store"?"admin":"store")}
                ...
                {view==="admin"?<><Grid size={13}/>Store View</>:<><Zap size={13}/>Admin</>}
              </button>
              {view==="admin" && (
              ...
              <button onClick={()=>setView(v=>v==="store"?"admin":"store")}
                ...
                {view==="admin"?<Grid size={16}/>:<Zap size={16}/>}
              </button>
            </div>
          </div>
        </div>
