...
function ProductCard({ product, onEdit, onDelete, isAdmin, darkMode, settings }) {
  ...
  return (
    <div className="relative flex flex-col overflow-hidden transition-all duration-300 group cursor-pointer"
      ...
      {/* Image */}
      <div className="relative overflow-hidden" style={{ aspectRatio:"1/1", background: darkMode?"#2d2d44":"#f9fafb" }}>
        ...
        {isAdmin && (
          <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            <button onClick={()=>onEdit(product)} className="w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-md">
              <Edit2 size={10} className="text-gray-700"/>
            </button>
            <button onClick={()=>onDelete(product.id)} className="w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-md">
          ...
          <div className="flex flex-wrap gap-1">
            {product.tags.split(",").map(t=>t.trim()).filter(Boolean).slice(0,3).map(tag=>(
              <span key={tag} className="text-[9px] px-1.5 py-0.5 rounded-full"
                style={{ background:"rgba(102,126,234,0.15)", color:"#667eea" }}>#{tag}</span>
            ))}
          </div>
        )}

        {/* Buttons row - Shop + WhatsApp */}
        <div className="flex gap-1.5 mt-1">
          <div className="flex-1"><ShopBtn url={product.affiliateLink}/></div>
