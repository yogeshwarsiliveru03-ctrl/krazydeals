...
function ProductDetailPopup({ product, onClose, darkMode, settings }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      onClick={onClose}>
      <div className="w-full sm:max-w-sm rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col max-h-[92vh]"
        style={{ background:bg, animation:"slideUp 0.35s cubic-bezier(0.32,0.72,0,1)" }}
        ...
        <div className="relative" style={{ aspectRatio:"1/1" }}>
          ...
          <button onClick={onClose}
            className="absolute top-3 right-3 w-9 h-9 rounded-full flex items-center justify-center shadow-lg transition-all duration-200 hover:scale-110"
          ...
          {product.discount>0 && (
            <div className="absolute top-3 left-3 px-2 py-1 rounded-lg text-white font-black text-[12px]"
          ...
          <div className="absolute bottom-3 right-3 px-2 py-1 rounded-lg text-white text-[10px] font-bold"
        ...
        <div className="p-5 flex flex-col gap-3 overflow-y-auto">
          ...
          {/* Name */}
          <h2 className="text-[16px] font-bold leading-snug" style={{ color:textColor }}>{product.name}</h2>

          {/* Description */}
          {product.description && (
            <p className="text-[13px] leading-relaxed" style={{ color:subText }}>{product.description}</p>
          )}

          {/* Rating */}
          {product.rating>0 && (
            <div className="flex items-center gap-2">
              <span className="text-[12px] font-bold text-white px-2 py-0.5 rounded flex items-center gap-1" style={{ background:"#388e3c" }}>
                {product.rating} <Star size={10} className="fill-white text-white"/>
              </span>
              {product.reviews>0 && <span className="text-[12px]" style={{ color:subText }}>{product.reviews?.toLocaleString()} reviews</span>}
            </div>
          )}

          {/* Price */}
          <div className="flex items-baseline gap-2 flex-wrap">
            <span className="text-[24px] font-black" style={{ color:textColor }}>Rs.{product.price?.toLocaleString()}</span>
            {product.originalPrice>product.price && (
              <span className="text-[14px] line-through" style={{ color:subText }}>Rs.{product.originalPrice?.toLocaleString()}</span>
            )}
            {product.discount>0 && (
          ...
          <div className="flex gap-2 mt-1">
            <a href={product.affiliateLink} target="_blank" rel="noopener noreferrer" className="flex-1">
              <button className="w-full py-3 rounded-2xl text-[13px] font-bold text-white transition-all duration-200 hover:opacity-90"
...
function ProductCard({ product, onEdit, onDelete, isAdmin, darkMode, settings }) {
  ...
  return (
    <>
      {showDetail && (
        <ProductDetailPopup product={product} onClose={()=>setShowDetail(false)} darkMode={darkMode} settings={settings}/>
      )}
      ...
      <div className="relative flex flex-col overflow-hidden transition-all duration-300 group"
        ...
        <div className="p-2.5 flex flex-col gap-1.5 cursor-pointer" onClick={()=>setShowDetail(true)}>
...
export { PLATFORMS, detectPlatform, CAT_THEMES, CATEGORIES, ADMIN_PASSWORD, defaultProducts, FloatingOrbs, DealTicker, Stars, BADGE_STYLES, Badge, ShopBtn, ProductCard, SkeletonCard, CatPill, Field, ImageUploadField, ProductModal, StatsBar, SettingsPanel, ScrollReveal };